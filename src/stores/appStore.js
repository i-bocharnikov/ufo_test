...
class AppStore {
  ...
  async initialise(t) {
    ...
    let result = await checkConnectivity() && await this.register(t) && await this.loadRemoteData(t)
    if (!result) {
  ...
  async connect(t, code): Promise<boolean> {
    if (await checkConnectivity()) {
      if (!await registerStore.connect(t, code)) {
        return false
      }
  ...
  async disconnect(t): Promise<boolean> {
    confirm(t('global:confirmationTitle'), t('register:disconnectConfirmationMessage'), async () => {
      if (await checkConnectivity()) {
