...
class OTAKeyStore {
    ...
    onOtaVehicleDataUpdated = async otaVehicleData => {
        try {
            ...
            this.doorsLocked =
                otaVehicleData.doorsLocked === true ? true : false;
            this.engineRunning =
                otaVehicleData.engineRunning === true ? true : false;
    ...
    async getKeyAccessDeviceIdentifier(
        ...
        try {
            await this.otaKeyLogger({
                ...
                code: codeTypes.SUCCESS,
                message: `-> this.ota.getAccessDeviceToken(${String(
                    force
                )}, ${String(showError)}) start`,
            });
        ...
        try {
            ...
            await this.otaKeyLogger({
                ...
                message: `<- this.ota.getVehicleData(${String(
                    ...
                    error.code
                        ? i18n.t(`otaKeyNativeErrors:${error.code}`) ||
                          error.message
                        : error.message
                }`,
                description: error,
    ...
    async getUsedKey(showError = true): Promise<boolean> {
        try {
            ...
            await this.otaKeyLogger({
                ...
                action: 'getUsedKey',
                code: codeTypes.SUCCESS,
                message: `<- this.ota.getUsedKey(${String(
                    showError
                )}) return key ${key.keyId}`,
                description: this.key,
            });
    ...
    async switchToKey(showError = true): Promise<boolean> {
        try {
            await this.otaKeyLogger({
                ...
                action: 'switchToKey',
                code: codeTypes.SUCCESS,
                message: `-> this.ota.switchToKey(${JSON.stringify(
                    this.key
                )}, ${String(showError)}) start`,
            });
            ...
            await this.otaKeyLogger({
                ...
                action: 'switchToKey',
                code: codeTypes.SUCCESS,
                message: `<- this.ota.switchToKey( ${String(
                    showError
                )}) return key ${this.key.keyId}`,
                description: this.key,
            });
    ...
    async connect(
        ...
        try {
            await this.otaKeyLogger({
                ...
                code: codeTypes.SUCCESS,
                message: `-> this.ota.connect(${String(
                    showNotification
                )}, ${String(showError)}) start`,
            });
            const result =
    ...
    async lockDoors(
        ...
        try {
            await this.otaKeyLogger({
                ...
                action: 'lockDoors',
                code: codeTypes.SUCCESS,
                message: `-> this.ota.lockDoors(${String(
                    requestVehicleData
                )}, ${String(showError)},) start`,
            });
